...
func NewCheckeredTexture(scale float64, c1, c2 Vec3) *Texture {
	t := Texture(&CheckeredTexture{InvScale: 1 / scale, Even: NewSolidColor(c1), Odd: NewSolidColor(c2)})
	return &t
}
func NewCheckeredTextureFromTexture(scale float64, even, odd *Texture) *Texture {
	t := Texture(&CheckeredTexture{InvScale: 1 / scale, Even: even, Odd: odd})
	return &t
}
